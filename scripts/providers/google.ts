...
const GOOGLE_MULTIMODAL_MODELS = ["gemini-3.1-pro-image-preview", "gemini-3.1-flash-image-preview"];
...
export async function generateImage(
  ...
  if (isGoogleImagen(model)) {
    if (args.referenceImages.length > 0) {
      throw new Error(
        "Reference images are not supported with Imagen models. Use gemini-3.1-pro-image-preview or gemini-3.1-flash-image-preview."
      );
    }
  ...
  if (!isGoogleMultimodal(model) && args.referenceImages.length > 0) {
    throw new Error(
      "Reference images are only supported with Gemini multimodal models. Use gemini-3.1-pro-image-preview or gemini-3.1-flash-image-preview."
    );
  }
